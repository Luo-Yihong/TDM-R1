    ...
    def update(self, prompts, rewards, type='grpo'):
        ...
        for prompt in unique:
            ...
            if type=='grpo':
                ...
                grpo_advantages = (prompt_rewards - mean) / std
                binarized = np.where(grpo_advantages >= 0, 1.0, -1.0)
                binarized_mean = np.mean(binarized)
                advantages[prompts == prompt] = binarized - binarized_mean
            elif type=='rwr':
                # advantages[prompts == prompt] = (prompt_rewards - mean) / std
