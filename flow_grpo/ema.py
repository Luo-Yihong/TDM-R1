...
class EMAModuleWrapper:
    ...
    @torch.no_grad()
    def step(self, parameters: Iterable[torch.nn.Parameter], optimization_step, decay = None):
        parameters = list(parameters)
    ...
    @torch.no_grad()
    def step_from_tensors(
        self,
        source_tensors: Iterable[torch.Tensor],
        decay: float,
        optimization_step: int,
    ) -> None:
        """EMA update from detached tensors (e.g. another wrapper's ``ema_parameters``)."""
        one_minus_decay = 1 - decay
        if (optimization_step + 1) % self.update_step_interval != 0:
            return
        source_tensors = list(source_tensors)
        for ema_parameter, source in zip(self.ema_parameters, source_tensors, strict=True):
            if ema_parameter.device == source.device:
                ema_parameter.add_(one_minus_decay * (source - ema_parameter))
            else:
                source_copy = source.detach().to(ema_parameter.device)
                source_copy.sub_(ema_parameter)
                source_copy.mul_(one_minus_decay)
                ema_parameter.add_(source_copy)
                del source_copy
