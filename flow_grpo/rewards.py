...
_HPSV3_SCORER_SINGLETON = None


def _get_hpsv3_scorer(device=None):
    """Singleton-cached HPSv3 scorer (Qwen2-VL-7B based, expensive to construct).

    `device=None` defers GPU placement to the first `__call__` (loads on CPU
    initially). Pass an explicit device to pin immediately. If the singleton
    already exists and an explicit device is given, the model is migrated.
    """
    global _HPSV3_SCORER_SINGLETON
    if _HPSV3_SCORER_SINGLETON is None:
        from flow_grpo.hpsv3_scorer import HPSv3Scorer
        _HPSV3_SCORER_SINGLETON = HPSv3Scorer(device=device)
    elif device is not None:
        _HPSV3_SCORER_SINGLETON._move_to(device)
    return _HPSV3_SCORER_SINGLETON


def hpsv3_score(device):
    # Pass device through; the scorer will migrate from CPU on first call if
    # it was pre-initialized without a device, or just stay put if already there.
    scorer = _get_hpsv3_scorer(device=device)

    def _fn(images, prompts, metadata):
        scores = scorer(images, list(prompts))
...
def _hpsv2_score_diff(device):
    from flow_grpo.diff_scorers import HPSv2ScorerDiff

    scorer = HPSv2ScorerDiff(dtype=torch.float32, device=device)

    def _fn(images, prompts):
        return scorer.forward_grad(images, prompts)

    return _fn


def _pickscore_score_diff(device):
    from flow_grpo.diff_scorers import PickScoreScorerDiff

    scorer = PickScoreScorerDiff(dtype=torch.float32, device=device)

    def _fn(images, prompts):
        return scorer.forward_grad(prompts, images)

    return _fn


def multi_score_diff(device, score_dict):
    """Differentiable weighted reward.

    Returns a callable `fn(images, prompts) -> tensor[B]` that computes a weighted sum of the
    configured differentiable scorers. `images` must be a float tensor in [0, 1] of shape
    [B, 3, H, W] and may carry gradients. Keys in `score_dict` that do not have a differentiable
    implementation are silently skipped (with a print warning).
    """
    diff_score_functions = {
        "hpsv2": _hpsv2_score_diff,
        "pickscore": _pickscore_score_diff,
    }
    score_fns = {}
    active_weights = {}
    for score_name, weight in score_dict.items():
        if score_name not in diff_score_functions:
            print(f"[multi_score_diff] skipping '{score_name}' (no differentiable implementation)")
            continue
        score_fns[score_name] = diff_score_functions[score_name](device)
        active_weights[score_name] = weight

    if not score_fns:
        raise ValueError(
            f"multi_score_diff: none of the configured rewards ({list(score_dict.keys())}) "
            f"have a differentiable implementation (supported: {list(diff_score_functions.keys())})."
        )

    def _fn(images, prompts):
        total = None
        for score_name, fn in score_fns.items():
            weight = active_weights[score_name]
