...
from .sd3_sde_with_logprob import sde_step_with_logprob, eta_step_with_logprob
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if eta is None:
                ...
                log_prob = latents.clone().reshape(latents.shape[0], -1).mean(dim=1)
            else:
                # eta=0 reproduces the FlowMatch Euler (DDIM) step bit-equivalently;
                # eta=1 yields CM-style sampling (predict-x0 + fresh noise).
                latents, log_prob, _, _ = eta_step_with_logprob(
                    self.scheduler,
                    noise_pred.float(),
                    t.unsqueeze(0),
                    latents.float(),
